...
\begin{equation}
	...
	\end{bmatrix},
\end{equation}  
with $x_{\text{step}}$ and $y_{\text{step}}$ are the position estimates produced by the step-detection and $x_{\text{camera}}$ and $y_{\text{camera}}$ being the position estimate from the tracking camera. Since the z-position has only the measurement  \textcolor{red}{from the tracking camera}, no weighting is performed. \textcolor{red}{der letzte Satz ist nicht klar!das wurde bsiher nicht erklärt; von z gibts nur tracking camera oder?! bitte den satz einfach genauer schreiben)}
%\begin{figure}[h!]
%	\centering
