...
The results over 10 runs show a mean deviation of 0.66\,m and therefore a 55\,\% improvement in tracking accuracy when using the proposed algorithm compared to the use of on step-detection only. In Table \ref{tab1}  the mean deviation at each control point of the different algorithms is listed. In the case of high tracking confidence the mean deviation is 0.32\,m and therefore 50\% lower than in cases where the tracking confidence is low, showing that the camera significantly influences the position estimate results. 
...
\begin{tabular}{lcccc}
	...
	Low tracking confidence S. F. & $0.66\mathrm{m}$ & $0.68\mathrm{m}$ & $0.76\mathrm{m}$ & $0.54\mathrm{m}$  \\ \bottomrule
\end{tabular}
\end{table}
