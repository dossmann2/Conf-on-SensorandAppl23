...
While safety standards in firefighting are continuously improving, indoor operations in burning buildings still present a dangerous task for firefighters. At least 240 injuries and 10 deaths involving firefighters conducting firefighting operations in buildings were reported in the United States in 2022 \cite{atemschutzunfalle.eu2023}. To improve safety while performing such a dangerous task, knowing the exact position of firefighters in indoor environments can shorten rescue time of injured personnel or help firefighters avoiding dangerous situations. Such technologies can not only improve safety of the involved firefighters, but can provide  real-time data for so-called internet of emergency services applications, which aims to improve emergency response and disaster management \cite{damasevcius2023}. To determine the position of a person in indoor or GPS-denied environments, a technique called Pedestrian Dead Reckoning (PDR) is used. It relies on sensors such as accelerometers, gyroscopes, and magnetometers integrated into wearable devices, smartphones or smartwatches. By continuously tracking a pedestrian's step counts, stride length, and heading changes, PDR algorithms can calculate their relative displacement from a known starting point. Other means of PDR include simultaneous locating and mapping \cite{lu2019}, magnetic field mapping \cite{wang2016} or magnetic triangulation \cite{arumugam2020}.
...
\begin{equation}
	(a_{z} < 0) \wedge (a_{z} > 0).  %Größer Null etc.!!
\end{equation} 
Only if these two conditions are registered in the algorithm, a step can be finally counted.
...
\begin{equation}
	...
	\end{bmatrix},
\end{equation}  
where $x_{\text{step}}$ and $y_{\text{step}}$ are the position estimates produced by the step-detection and $x_{\text{camera}}$ and $y_{\text{camera}}$ being the position estimate from the tracking camera. Since the step-detection cannot estimate the z-position, only the measurement from the tracking camera is used, therefore no weighting is performed.
%\begin{figure}[h!]
%	\centering
...
\begin{figure}
	...
	\caption{Sensor assembly mounted on the firefighter equipment.}
	\label{fig:Assembly}
\end{figure}
...
In the second validation experiment, dynamic crouching, frequently employed in firefighting, has been tested. The test path, illustrated in the left diagram of Figure \ref{fig:path}, covers a total length of 12\,m and includes four 90$^\circ$ turns.
...
\bibitem{damasevcius2023}
R. Damaševičius, N. Bacanin, and S. Misra 
\newblock From Sensors to Safety: Internet of Emergency Services (IoES) for Emergency Response and Disaster Management.
\newblock {\em Journal of Sensor and Actuator Networks}, 12(3),
2023.
...
\newblock A {{mulit-IMU based self-contained pedestrian navigation algorithm}}.
...
T.~A. Brovko, S.~A. Serov, and I.~V. Korogodin.
