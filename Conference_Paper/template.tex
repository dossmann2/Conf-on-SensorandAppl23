...
\abstract{Knowing the exact position of firefighters in a building during an indoor firefighting operation is critical to improving the efficiency and safety of firefighters. To estimate the position of an individual in indoor or GPS denied environments an approach called Pedestriasn Dead Reckoning (PDR) is used. PDR describes the process of estimating position by using sensors like accelerometers and gyroscopes. One of the most common techniques of PDR is step-detection. However, in an application like firefighting, where more dynamic movements like crouching occur, the accuracy of a step-detection algorithm is not satisfactory. This paper presents a enhanced PDR algorithm that augments this conventional PDR technique with a tracking camera as a secondary sensor. The position estimates of a zero-crossing step-detection algorithm and the tracking camera estimates are fused in a Kalman filter. A system prototype is designed for algorithm validation. The experimental outcomes  demonstrate that the inclusion of a secondary sensor leads to a substantial enhancement in overall accuracy. Particularly in tests involving dynamic crouching maneuvers overall accuracy is improved. }
