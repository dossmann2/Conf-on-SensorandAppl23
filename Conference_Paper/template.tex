...
Once  a step is registered as complete, the step length has to be added to the current position in the direction of movement. The method used for estimating the step length uses the connection between hip acceleration and step length \cite{weinberg2002} to estimate 
 \begin{equation}
...
Sensor-data fusion describes the process of using multiple sensor outputs to estimate the state of a system. An example for a simple sensor data fusion algorithm averaging the
...
Doing this for multiple objects and repeating this process every frame, the position and average velocity can be calculated. Figure \ref{fig:cmaera} shows the two aligned cameras as well as the cameras body coordinate frame.
...
\begin{itemize}
	\item Time since last step was detected. It is assumed that the accuracy of the estimated position by the step-detection algorithm deterioates with time if no successing step is detected. Three conditions are proposed: the time passed since the last detection event $t_{step} < 0.5\mathrm{s}$, $0.5\mathrm{s} \leq t_{step} < 3\mathrm{s}$ or $t_{step} \geq 3\mathrm{s}$.\\
...
To evaluate sensor and algorithm performance, tests on a predefined path are performed. To quantify the results, the deviation from the true path is evaluated at a set of control points. The control points represent changes in heading. This allows an easy comparison between the true path and the measured path. In the tests, a dynamic crouching movement often used in firefigthing is used to better evaluate the Step-detection and the Tracking Camera. To simulate obstruction of the tracking camera by dirt or heavy smoke, the tracking confidence is artificially reduced so that the step detection is highly favoured.\\

The path defined for the test has an overall length of $12\mathrm{m}$ with four $90\deg$ turns. The starting- and endpoint are identical. Ten test runs are performed where the individual completes the defined path in a crouching movement characteristic for firefighting operations. \\
...
\begin{figure}
	...
	\begin{subfigure}[b]{0.49\textwidth}
		\centering
		\hspace{-2.5cm}
		\includegraphics[width=1.1\textwidth]{Path.eps}
...
A second test is performed where a path with an overall length of about $33\mathrm{m}$ is walked. The results show an increased accuracy of the step-detection when walking compared to crouching movements, therefore increasing overall accuracy.
\end{table}
...
The test results show that by adding a secondary sensor a higher position accuracy can be achieved compared to a system relying only with step-detection. Especially in situations where more dynamic movements like crouching occur, the step-detection method produces unaccurate results. Therefore to improve the accuracy of this design for firefighting applications, an improved tracking camera capable of operating in near zero visibility environments has to be developed. By using multiple IMUs the accuracy could be further improved. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Patents}
