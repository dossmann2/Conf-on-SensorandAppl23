...
\abstract{To determine the position of an individual in an indoor environment, in most cases, an approach called Pedestrian Dead Reckoning (PDR) is used. PDR describes the process of estimating the position by using sensors like accelerometers and gyroscopes. One of the most common techniques of PDR is step-detection. However, in an application like firefighting, where more dynamic movements like crouching occur, this is not sufficient. This paper presents a PDR approach which enhances a traditional step-detection using vertical acceleration with a tracking camera as a secondary sensor and fusing the sensor data in a Kalman filter. A prototype of the design is designed and tested. Results show that using a secondary sensor can improve overall accuracy.  }
