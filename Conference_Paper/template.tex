...
For a realistic validation of the sensor setup and the  algorithms,  tests on predefined paths are performed.
...
First, in order to validate the performance during regular walking scenarios, a g 33\,m long path is tested. The right diagram in Figure \ref{fig:path} shows the estimated position of the different  algorithms compared to the true path. While at the beginning of the test all three  deliver accurate results, the step-detection deviates strongly after the first heading change. The proposed sensor fusion is able to stay close to the real path and deliver the best results most of the time.
The tracking camera alone, however, delivers the best result in the middle of the experiments. This is due to the fact, that here the step-detection shows a big error dragging also the sensor fusion algorithm away from the real path. This indicates that a fine tuning still can improve the results.
...
As dynamic crouching is often used in firefighting, this scenarios is tested in the second validation experiment. To simulate obstruction of the tracking camera by dirt or heavy smoke, the tracking confidence is artificially reduced so that the step detection is highly favored. The path defined for the test has an overall length of 12\,m with four 90$^\circ$ turns, as shown in the left diagram in Figure \ref{fig:path}. The starting- and endpoint are identical and 10 test runs  are performed. Clearly, the step detection alone performs the worst in this scenario.
% where the individual completes the defined path in a crouching movement characteristic for firefighting operations.
The results over 10 runs show a mean deviation of 0.66\,m and therefore a 55\,\% improvement in tracking accuracy when using the proposed algorithm compared to the use of on step-detection only. In Table \ref{tab1}  the mean deviation at each control point of the different algorithms is listed. In the case of high tracking confidence the mean deviation is 0.32\,m and therefore 50\% lower than in cases where the tracking confidence is low, showing that the camera significantly influences the position estimate results. With a mean deviation of 0.3\,m the tracking camera alone performs slightly better than the sensor fusion algorithm, however it has to be acknowledged that these results are performed in perfect conditions for the optical sensor. 

\textcolor{red}{ nach der figure zu unteilen wäre hier intessant, auch die ergebnisse der 10 läufe zu zeigen, wenn man nur die kamera nimmt, denn die siehtim bild nicht viel schlechter aus. SO hat man als Leser das Gefühl, sie verheimlichen etwas! HAben wir Daten, um das noch hinzuzufügen?!Selbst wenn es leicht besser wäre, wäre es ja in Ordnung.}
\begin{figure}[t!]
	\centering
...
\begin{tabular}{lcccc}
	...
	Step-Detection                              & $1.60\,\mathrm{m}$ & $2.45\,\mathrm{m}$ & $2.44\,\mathrm{m}$ & $1.80\,\mathrm{m}$  \\
	...
	Tracking Camera & $0.27\,\mathrm{m}$ & $0.43\,\mathrm{m}$ & $0.24\,\mathrm{m}$ & $0.29\,\mathrm{m}$ \\ \bottomrule
\end{tabular}
\end{table}
