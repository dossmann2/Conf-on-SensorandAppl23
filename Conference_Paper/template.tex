...
\Author{Tobias Augustin $^{1}$*\orcidA{}, Daniel Ossmann $^{1}$\orcidB{} }
...
\abstract{Knowing the exact position of firefighters in a building during an indoor firefighting operation is critical to improving the efficiency and safety of firefighting. In Indoor or GPS denied environments an approach called Pedestriasn Dead Reckoning (PDR) is used. PDR describes the process of estimating position by using sensors like accelerometers and gyroscopes. One of the most common techniques of PDR is step-detection. However, in an application like firefighting, where more dynamic movements like crouching occur, the accuracy of the position estimate is not satisfactory. This paper presents a PDR approach which enhances a traditional step-detection using vertical acceleration with a tracking camera as a secondary sensor. The sensor data is fused in in a Kalman filter. A prototype of the system is designed to test the algorithm. Results show that using a secondary sensor improves overall accuracy.  }
